	...
	for !r.IsStop() {
		...
		if helper.null {
			...
			if atomic.CompareAndSwapInt32(&helper.init, 0, 1) {
				helper.udpMsgQue = newUdpAccept(r.conn, r.handler, addr)
				helper.null = false
			}
			helper.Unlock()
		}
...
func newUdpAccept(conn *net.UDPConn, handler IMsgHandler, addr *net.UDPAddr) *udpMsgQue {
	msgque := udpMsgQue{
		msgQue: msgQue{
...
func newUdpListen(conn *net.UDPConn, handler IMsgHandler, addr string) *udpMsgQue {
	msgque := udpMsgQue{
		msgQue: msgQue{
			id:        atomic.AddUint32(&msgqueId, 1),
			handler:   handler,
